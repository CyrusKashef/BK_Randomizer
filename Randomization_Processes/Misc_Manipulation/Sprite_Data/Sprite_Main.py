...
class Sprite_Manipulation_Class():
    '''Sprite manipulation class'''
    def __init__(self, seed_val, file_dir, randomized_rom_path):
        '''Initializes the sprite manipulation class'''
        ...
        self._pointers_start = 0x9DC8
        self._pointers_end = 0xA0E0
        self._sprite_address_dict = {}
        self._sprite_dict = read_json(f"{self._file_dir}Randomization_Processes\\Misc_Manipulation\\Sprite_Data\\Sprites.json")
    
    def _grab_compressed_file(self, pointer_str):
        '''Uses the pointer to find the beginning and end of a sprite file and extracts it'''
    ...
    def _place_compressed_files(self):
        '''Replaces the sprite pointers with the new sprite file'''
...
#             print("~~~~")
            ...
            curr_pointer_file = self._sprite_address_dict[pointer_str]
...
#                 print(f"Address End Hex: {address_end_hex}")
        if((mm_rand_rom[self._pointers_end + 8] != 0x0) or
           (mm_rand_rom[self._pointers_end + 9] != 0x57) or
           (mm_rand_rom[self._pointers_end + 10] != 0x8A) or
           (mm_rand_rom[self._pointers_end + 11] != 0xC8)):
    ...
    def _sprite_manip_main(self):
        '''Runs through the functions of extracting, shuffling, and reinserting the sprite files'''
        # Main Categories
        sprite_categories = ["Talking"]
        pointer_dict = {}
        for category in self._sprite_dict:
            category_pointer_list = []
            # Extract Compressed Files
            pointer_dict[category] = []
            for pointer_str in self._sprite_dict[category]:
        ...
        for category in self._sprite_dict:
            ...
            if(category in sprite_categories):
                category_pointer_list = self._shuffle_list(category_pointer_list)
            # Assign New Files
            list_counter = 0
            for pointer_str in self._sprite_dict[category]:
                self._sprite_address_dict[pointer_str.lower()] = category_pointer_list[list_counter]
                list_counter += 1
